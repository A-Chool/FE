...
export const loadTeamBoard = (teamId) => {
  ...
  return function (dispatch, getState, { history }) {
    ...
    axios.get('https://13.209.21.57:443/api/user/teamBoard', bucket)
    .then((res) => {
...
export const setWeekTeamBoard = (teamId) => {
  return function (dispatch, getState, { history }) {
    const myToken = getCookie("Authorization");
    axios.get(`https://13.209.21.57:443/api/user/teamBoard/${teamId}`
    ,{headers : {"Authorization" : `Bearer ${myToken}`}}
    )
    .then((res) => {
...
export const editGroundRule = (teamId, groundRule, weekId) => {
  ...
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "put",
      url: `https://13.209.21.57:443/api/user/teamBoard/groundRule/${teamId}`,
      data: {
        groundRule
      },
      headers: {Authorization : `Bearer ${myToken}`},
    ...
    .then(() => {
      ...
      axios.get('https://13.209.21.57:443/api/user/teamBoard'
      ,{headers : {"Authorization" : `Bearer ${myToken}`}}
...
export const addTodoList = (teamId, todoContent, weekId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "post",
      url: `https://13.209.21.57:443/api/user/teamBoard`,
      data: {
      teamId : teamId,
...
export const deleteTodoList = (todoId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "delete",
      url: `https://13.209.21.57:443/api/user/teamBoard/${todoId}`,  
      headers: {
...
export const checkTodoList = (todoId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "put",
      url: `https://13.209.21.57:443/api/user/teamBoard/check/${todoId}`,
      data: {
      },
      headers: {Authorization: `Bearer ${myToken}`},
    })
...
export const editWorkSpace = (teamId, workSpace, weekId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "put",
      url: `https://13.209.21.57:443/api/user/teamBoard/workSpace/${teamId}`,
      data: {
        workSpace
      },
      headers: {Authorization: `Bearer ${myToken}`},
