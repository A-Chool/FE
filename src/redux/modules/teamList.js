...
return function (dispatch, getState, { history }) {
  ...
  axios.get('https://achool.shop/api/admin/teams/week'
...
export const addTeamList = (teamName, weekId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "post",
      url: `https://achool.shop/api/admin/teams/${weekId}`,
      data: {teamName},
      headers: {
...
export const deleteTeamList = (teamId, weekId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "delete",
      url: `https://achool.shop/api/admin/teams/${teamId}`,     
      headers: {
      Authorization: `Bearer ${myToken}`
      },
    })
...
export const loadMemberList = (weekId) => {
  return function (dispatch, getState, { history }) {
    const myToken = getCookie("Authorization");
    axios.get(`https://achool.shop/api/admin/noMember/${weekId}`
    ,{headers : {"Authorization" : `Bearer ${myToken}`}}
    )
    .then((res) => {
...
export const addMemberList = (userId, teamId, weekId) => {
  ...
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "post",
      url: `https://achool.shop/api/admin/member/${weekId}`,
      data: {
      userId : userId,
      teamId : teamId,
    },
...
export const deleteMemberList = (memberId, userId, weekId) => {
  return function (dispatch, getState, {history}) {
    ...
    axios({
      method: "delete",
      url: `https://achool.shop/api/admin/member/${memberId}`,
      // data : {userId : userId},     
      headers: {
      Authorization: `Bearer ${myToken}`
      },
